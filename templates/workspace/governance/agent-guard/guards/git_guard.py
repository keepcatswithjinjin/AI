    ...
    for segment in command_segments(command):
        ...
        if subcommand == "worktree" and rest:
            ...
            if action == "add" and git_policy.get("deny_direct_worktree_add", True):
                return "Direct git worktree add is blocked. Use D:\\Col\\scripts\\new-worktree.cmd so path, branch, and Serena rules are enforced."
            if action == "remove" and git_policy.get("deny_direct_worktree_remove", True):
                return "Direct git worktree remove is blocked. Use D:\\Col\\scripts\\remove-worktree.cmd so workspace state and Serena indexes are cleaned safely."
