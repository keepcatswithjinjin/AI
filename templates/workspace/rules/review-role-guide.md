...
> 当用户明确说“你现在属于 review 职责”或要求上线前 Review 时，Agent 进入本角色。本角色独立于后端、前端实现角色：接受当前代码事实，审查本次 diff，不承担实现、冲突解决或发布操作。它不同于 `first-pass-code-review.md` 定义的提交前人工代码导航；后者不产生 Review 结论。
